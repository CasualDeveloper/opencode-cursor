...
test("unanchored redacted reasoning fails explicitly, regardless of blob arrival order", async () => {
  for (const checkpointFirst of [false, true]) {
    const backend = await peer((message, stream) => {
      ...
      stream.write(end());
      stream.write(checkpointFirst ? checkpoint : blob);
      stream.write(checkpointFirst ? blob : checkpoint);
      stream.end(close());
    });
    await expect(model(backend.url).doGenerate({ prompt })).rejects.toThrow(
      "Cursor opaque reasoning has no unique emitted assistant anchor",
    );
  }
});
