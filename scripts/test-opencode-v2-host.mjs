  ...
  const afterRoots = () => {
    try {
      ...
      if (phase === "auto-compact-next") {
        ...
        assert.match(observation.text, /AUTO_COMPACTION_SEED/);
      ...
      if (phase === "compacting") {
        // This pinned host renders its own summary task as a user message and
        // retains recent messages separately. The adapter must preserve that input.
        assert.equal(observation.tools.length, 0);
        assert.match(observation.text, /nonce-a-from-real-host-tool/);
        assert.match(
          observation.text,
          /\[Tool error\]: Permission denied: read/,
        );
...
try {
  ...
  await writeFile(
    ...
    JSON.stringify({
      plugins: [resolve("test/fixtures/v2-host-plugin.ts")],
  ...
  const launchHost = () => {
    child = spawn(
      resolve("node_modules/@opencode-ai/cli/bin/opencode2.exe"),
    ...
    until(async () => {
      try {
        return (await request("/api/plugin")).data?.some(
          (item) =>
            item.id === "test.cursor-host-boundary" && item.status === "active",
        );
      } catch {
        return false;
      }
  ...
  await until(async () => {
    const exported = (await request(`${sessionPath}/export`)).data;
  ...
  const toolParts = async (path) =>
    (await request(`${path}/export`)).data.messages.flatMap((message) =>
      message.type === "assistant"
        ? message.content.filter((part) => part.type === "tool")
        : [],
    );
  ...
  for (const action of ["rejection", "interruption"]) {
    ...
    else await request(`${path}/interrupt`, { continue: false });
    await idle(path);
  ...
  await request(`${interruptedPath}/interrupt`, { continue: false });
  ...
  await idle(interruptedPath);
  assert.equal(
    (await request(`${interruptedPath}/export`)).data.info.outcome,
    "succeeded",
  );
  ...
  assert.equal(
    (await request(`${invalidPath}/export`)).data.info.outcome,
    "succeeded",
  );
  ...
  assert.equal(
    (await request(`${steeredPath}/export`)).data.info.outcome,
    "succeeded",
  );
  ...
  await idle(imagePath);
  assert.equal(
    (await request(`${imagePath}/export`)).data.info.outcome,
    "succeeded",
  );
  ...
  assert.equal(
    (await request(`${toolImagePath}/export`)).data.info.outcome,
    "succeeded",
  );
  ...
  await idle(forkPath);
  assert.equal(
    (await request(`${forkPath}/export`)).data.info.outcome,
    "succeeded",
  );
  assert.deepEqual(
    (await request(`${sessionPath}/export`)).data.messages,
    exported.messages,
  );
  ...
  assert.equal(
    (await request(`${sessionPath}/export`)).data.info.outcome,
    "succeeded",
  );
  ...
  await idle(usagePath);
  const usageExport = (await request(`${usagePath}/export`)).data;
  ...
  assert.equal(
    (await request(`${autoPath}/export`)).data.info.outcome,
    "succeeded",
  );
  const beforeRestart = (await request(`${forkPath}/export`)).data;
  ...
  await hostReady();
  assert.deepEqual(
    (await request(`${forkPath}/export`)).data.messages,
  ...
  console.log(
    "OpenCode beta-18050 host acceptance passed: delayed parallel tools, permission waits/rejection, question dismissal, interrupted side effects, checkpoint steering, invalid input, usage, images, forks, signed/opaque reasoning restart replay, manual/automatic compaction, cold replay, and graceful completion.",
  );
} catch (error) {
