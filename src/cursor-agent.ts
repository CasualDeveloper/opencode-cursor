    ...
    for (const key of this.checkpointRoots) {
      ...
      const content = root.map((value): JsonValue => {
        ...
        if (
          ...
          throw new Error("Cursor opaque reasoning model mismatch");
        if (part?.type !== "tool-call" || typeof part.toolCallId !== "string")
          return value;
        const call = this.calls.get(part.toolCallId);
        return call ? { ...(value as JsonObject), toolCallId: call.id } : value;
      });
      ...
      const matches = [...this.outputEntries].filter((index) => {
        ...
        return (
          entry?.role === "assistant" &&
          assistantDigest(entry.content) === annotation.digest
        );
      });
      if (matches.length !== 1)
        throw new Error(
